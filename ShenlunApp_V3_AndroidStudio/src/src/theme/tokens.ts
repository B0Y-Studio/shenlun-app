...
export const fonts = {
  // 思源宋体已被移除以减小 APK 体积 24MB。
  // serif 改为系统衬线字体（Android Noto Serif / iOS Times New Roman）。
  serif: {
    regular: 'serif',
    medium:  'serif',
    bold:    'serif',
  ...
  kai: {
    regular: 'serif',   // 楷体 Android 无原生 fallback，先用 serif
    bold:    'serif',
  },
  sans: {
    regular: 'sans-serif',
...
// 把 Date 转成 V3 顶部右侧 4 字表达（公历）
// line1="X月"（汉字月），line2="汉字日"（公历日转汉字）
export function buildHeaderDate(d: Date): { line1: string; line2: string } {
  const m = d.getMonth() + 1;
  const day = d.getDate();
  return { line1: `${m}月`, line2: solarDayCn(day) };
}
